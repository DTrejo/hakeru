...
function acceptButtonClick(e){
  e.preventDefault();
  socket.send(JSON.stringify({type:"accept_task", data: {task_id: $(this).attr("task_id")}}));
}

function deleteButtonClick(e){
  e.preventDefault();
  socket.send(JSON.stringify({type:"delete_task", data: {task_id: $(this).attr("task_id")}}));
}

function giveUpClick(e){
  e.preventDefault();
  socket.send(JSON.stringify({type:"give_up", data:{task_id: $(this).attr("task_id")}}));
}
  ...
  $("#msg").keypress(function(e){
    if(e.keyCode == 13) {
      if(!e.ctrlKey && !e.shiftKey){ 
        ...
        socket.send(JSON.stringify({type: identifyMessageType(msgText), data: {msg: msgText, group: identifyTaskGroup(msgText)}}));
        $("#msg").val("");
        scrollPaneToBottom($("#chatwrap .innercontent"), true, true);
        return false;
      }
